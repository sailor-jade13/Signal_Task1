...
class CNN_LSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim):
        ...
        # 对LSTM层的输出结果进行卷积变形([64, 1, 1, 128]→[64, 16, 1, 52])
        self.conv2 = nn.Sequential(
            # 在BN层之前将conv2d的偏置bias设置为false
            nn.Conv2d(1, 32, (1, 7), stride=1,bias=False),
            #BN层输入通道数应与前一层Conv2d的输出通道数一致
            nn.BatchNorm2d(32),
            nn.PReLU(),
            # nn.Dropout(0.5),
            # 在BN层之前将conv2d的偏置bias设置为false
            nn.Conv2d(32, 16, (1, 7), stride=1, bias=False),
            nn.BatchNorm2d(16),
            nn.PReLU(),
        )
    ...
    def forward(self, x):
        x1 = self.conv1(x) #[64, 16, 1, 52]
        x2, (h_n, h_c) = self.lstm(torch.squeeze(x, dim=2), None)
        x2 = torch.unsqueeze(x2, dim=2) #[64, 1, 1, 128]
        x2 = self.conv2(x2) #[64, 16, 1, 52]
        # 不知道expand方法是否可行
        # x2 = x2.expand(64,16,1,52)
        # print(x1.shape)
        # print(x2.shape)
        x = torch.cat([x1, x2], dim=2)
        # print(x.shape) #[64, 16, 2, 52]
        # print(x.view(x.shape[0], -1).shape)  #(64,16*2*52)
...
if __name__ == '__main__':
    ...
    layer_dim = 1
    output_dim = 8
    net = CNN_LSTM(input_dim,hidden_dim,layer_dim,output_dim)
    print(net)
    ...
    #     #测试准确度
    #     test_acc = test_accuracy(test_loader,my_convnet)
