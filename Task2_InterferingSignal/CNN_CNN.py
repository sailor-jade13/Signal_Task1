...
class CNN2(nn.Module):
    def __init__(self,class_num=2):
        ...
        ### 分别用不同 变量名 定义每个并联模块 ###
        for iii in range(2):
            locals()["block" + str(iii + 1)] = []  # Variable name: block1、block2
            locals()["block" + str(iii + 1)] += [torch.nn.Conv2d(1, 128, (1, 3), 1, bias=False)]
            locals()["block" + str(iii + 1)] += [torch.nn.BatchNorm2d(128)]
            locals()["block" + str(iii + 1)] += [torch.nn.PReLU()]
            locals()["block" + str(iii + 1)] += [torch.nn.Conv2d(128, 64, (1, 5), stride=1, padding=(0, 1), bias=False)]
            locals()["block" + str(iii + 1)] += [torch.nn.BatchNorm2d(64)]
            locals()["block" + str(iii + 1)] += [torch.nn.PReLU()]
            locals()["block" + str(iii + 1)] += [torch.nn.Conv2d(64, 32, (1, 7), stride=1, padding=(0, 1), bias=False)]
            locals()["block" + str(iii + 1)] += [torch.nn.BatchNorm2d(32)]
            locals()["block" + str(iii + 1)] += [torch.nn.PReLU()]
            locals()["block" + str(iii + 1)] += [torch.nn.Conv2d(32, 16, (1, 7), stride=1, padding=(0, 1), bias=False)]
            locals()["block" + str(iii + 1)] += [torch.nn.BatchNorm2d(16)]
            locals()["block" + str(iii + 1)] += [torch.nn.PReLU()]
        ...
        # self.classifier = ClassBlock(8, class_num=class_num)
